...
data.frame(
  `Exponentiated Coefficent` = round(tmp, 2)
) |>
  knitr::kable(caption = "Exponentiated coefficients represent the per-unit additive change in odds for each business type within an FSA.")
...
The scatter plots in Figure \@ref(fig:scatterplots) express the relationship between the response and the independent variables. In this cause the dispensary count and the type of business. The importance of the scatter plots is to help the reader visualize our findings and division of FSA's with the likelihood of having 3 or more dispensaries.

## The Spatial Visualizations

The spatial visualizations express visually how the FSA's are divided, the geographic location of a particular dispensary as well as the volume of particular businesses within a FSA. The map is divided by these forward sortation areas of Toronto from the year 2005[@UofTFSA]. The reason for the specific mentioning of the boundaries is because over time these FSA's can change but the team cannot find access to the new boundary geometries.
